...
router.get('/',verify,(request, response)=> {
    ...
    .then((assetQueryResult) => {
            ...
            console.log('assetresult view'+JSON.stringify(assetQueryResult.rows));
            let projId ;
            if(assetQueryResult.rowCount > 0)
            {
                projId = assetQueryResult.rows[0].Project_Department__c;
                console.log('projectId '+projId);
                 response.render('assetRequistionForms',{objUser : objUser, name:request.user.name, email : request.user.email, projId : projId, assetList : assetQueryResult.rows }); 
            }
            else
...
router.get('/assetEditDetails',(request, response) =>{
    ...
    let qyr='SELECT asset.id, asset.sfid as sfid,asset.name as name ,act.sfid as actsfid, asset.GST__c,asset.Requested_Closure_Plan_Date__c,asset.Requested_Closure_Actual_Date__c,asset.Project_Department__c, '+
...
router.post('/updateasset',(request,response)=>{
    ...
    const { assetName,activityCode,paymentStatus,status,payement,receiverName,receivedQuantity,assetid,quotations,reason,pricing,deliveryPlace,deliveryTime,deliveryCost,attachment} = request.body;
